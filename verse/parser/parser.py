...
class CondValCase(CustomIR):
    ...
    def __eq__(self, o) -> bool:
        if o == None or len(self.cond) != len(o.cond):
            return False
        return all(
            ControllerIR.ir_eq(sc, oc) for sc, oc in zip(self.cond, o.cond)
        ) and ControllerIR.ir_eq(self.val, o.val)
...
class Reduction(CustomIR):
    ...
    def __eq__(self, o) -> bool:
        ...
        return (
            self.op == o.op
            and self.it == o.it
            and ControllerIR.ir_eq(self.expr, o.expr)
            and ControllerIR.ir_eq(self.value, o.value)
        )
...
class _Assert:
    ...
    def __eq__(self, o) -> bool:
        if o == None:
            return False
        return (
            len(self.pre) == len(o.pre)
            and all(ControllerIR.ir_eq(a, b) for a, b in zip(self.pre, o.pre))
            and ControllerIR.ir_eq(self.cond, o.cond)
        )
...
class ControllerIR:
    ...
    def dump(node, dump=False):
        ...
        if isinstance(node, Reduction):
            ...
            return (
                "{" + ", ".join(f"{k}: {ControllerIR.dump(v, dump)}" for k, v in node.items()) + "}"
            )
    ...
    def ir_eq(a: Optional[ScopeValue], b: Optional[ScopeValue]) -> bool:
        ...
        return ControllerIR.dump(a) == ControllerIR.dump(
            b
        )  # FIXME Proper equality checks; dump needed cuz asts are dumb
    ...
    def from_env(env):
        ...
        asserts = [
            (a.cond, a.label if a.label != None else f"<assert {i}>", merge_conds(a.pre))
            for i, a in enumerate(controller.asserts)
        ]
        asserts_veri = [
            Assert(
                Env.trans_args(copy.deepcopy(c), True), l, Env.trans_args(copy.deepcopy(p), True)
            )
            for c, l, p in asserts
        ]
        ...
        asserts_sim = [
            CompiledAssert(
                compile_expr(Env.trans_args(c, False)), l, compile_expr(Env.trans_args(p, False))
            )
            for c, l, p in asserts
        ]
    ...
    def trans_args(sv: ScopeValue, veri: bool) -> ScopeValue:
            ...
            if veri:
                ...
                for case in reversed(copy.deepcopy(cv).elems):
                    if len(case.cond) == 0:
                        ...
                        cond = (
                            ast.BoolOp(ast.And(), case.cond) if len(case.cond) > 1 else case.cond[0]
                        )
        ...
        def trans_reduction(red: Reduction, veri: bool):
            ...
            gen_expr = ast.GeneratorExp(
                expr, [ast.comprehension(ast.Name(red.it, ctx=ast.Store()), value, [], False)]
            )
...
def merge_if(test: ast.expr, trues: Env, falses: Env, env: Env):
    ...
    env.scopes[0].asserts = merge_assert(
        test, trues.scopes[0].asserts, falses.scopes[0].asserts, env.scopes[0].asserts
    )
...
def merge_if_single(
    test, true: ScopeValueMap, false: ScopeValueMap, scope: Union[Env, ScopeValueMap]
):
    def lookup(s, k):
        if isinstance(s, Env):
            return s.lookup(k)
            ...
            if not isinstance(lookup(scope, var), dict):
                if lookup(scope, var) != None:
                    raise NotImplementedError(
                        f"uncaught object assignment before attribute assignment: {var, lookup(scope, var)}"
                    )
                assign(scope, var, {})
            var_true_emp, var_false_emp, var_scope = (
                true.get(var, {}),
                false.get(var, {}),
                lookup(scope, var),
            )
            assert (
                isinstance(var_true_emp, dict)
                and isinstance(var_false_emp, dict)
                and isinstance(var_scope, dict)
            )
...
def merge_if_val(
    test, true: Optional[ScopeValue], false: Optional[ScopeValue], orig: Optional[ScopeValue]
) -> CondVal:
    ...
    true_emp, false_emp = (
        true == None or len(true.elems) == 0,
        false == None or len(false.elems) == 0,
    )
    if true_emp and false_emp:
        ...
        merge_true, merge_false = merge_cond(test, true), merge_cond(
            ast.UnaryOp(ast.Not(), test), false
        )
...
def is_main_check(node: ast.If) -> bool:
    ...
    return (
        ...
        and (
            check_comps(node.test.left, node.test.comparators[0])
...
def proc(node: ast.AST, env: Env) -> Any:
    ...
    # Expressions
        ...
        rest = [
            ast.Compare(proc_comps[i], node.ops[i + 1 : i + 2], proc_comps[i + 1 : i + 2])
        ...
        if isinstance(fun, ast.Attribute):
            if (
                isinstance(fun.value, ast.arg)
                and fun.value.arg == "copy"
                and fun.attr == "deepcopy"
            ):
                if len(node.args) > 1:
        ...
        if isinstance(node.value, ast.Call):
            warnings.warn(
                f'Effects of this call will not be included in the result: "{unparse(node.value)}"'
            )
        return None
