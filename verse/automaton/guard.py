...
class ValueSubstituter(ast.NodeTransformer):
    ...
    def visit_Attribute(self, node: ast.Attribute) -> Any:
        ...
        if isinstance(self.node, Reduction):
            return node
        if node == self.node:
            return ast.Name(id=self.val, ctx=ast.Load())
        return node
    ...
    def visit_Reduction(self, node: Reduction) -> Any:
        if node == self.node:
            if len(self.val) == 1:
                ...
                self.generic_visit(node)
                return ast.BoolOp(op=ast.Or(), values=self.val)
...
class GuardExpressionAst:
    ...
    def _evaluate_guard_hybrid(self, root, agent, disc_var_dict, cont_var_dict, track_map: LaneMap, agent_vars):
        if isinstance(root, ast.Compare):
            ...
            if isinstance(root.func, ast.Attribute):
                ...
                raise ValueError(
                    f"Node type {root.func} from {unparse(root.func)} is not supported"
                )
    ...
    def _handle_longitudinal_set(self, lane_seg: AbstractLane, position) -> List[float]:
        if lane_seg.type == "Straight":
            ...
            longitudinal_low = min(
                delta0[0] * lane_seg.direction[0], delta1[0] * lane_seg.direction[0]
    ...
    def _handle_lateral_set(self, lane_seg: AbstractLane, position: np.ndarray) -> List[float]:
        if lane_seg.type == "Straight":
            ...
            lateral_low = min(
                delta0[0] * lane_seg.direction_lateral[0], delta1[0] * lane_seg.direction_lateral[0]
            ) + min(
                delta0[1] * lane_seg.direction_lateral[1], delta1[1] * lane_seg.direction_lateral[1]
            )
            lateral_high = max(
                delta0[0] * lane_seg.direction_lateral[0], delta1[0] * lane_seg.direction_lateral[0]
            ) + max(
            ...
            dx = np.max(
                [position[0, 0] - lane_seg.center[0], 0, lane_seg.center[0] - position[1, 0]]
            )
            dy = np.max(
                [position[0, 1] - lane_seg.center[1], 0, lane_seg.center[1] - position[1, 1]]
            )
            r_low = np.linalg.norm([dx, dy])

            dx = np.max(
                [
                    np.abs(position[0, 0] - lane_seg.center[0]),
                    np.abs(position[1, 0] - lane_seg.center[0]),
                ]
            )
            dy = np.max(
                [
                    np.abs(position[0, 1] - lane_seg.center[1]),
                    np.abs(position[1, 1] - lane_seg.center[1]),
                ]
            )
            r_high = np.linalg.norm([dx, dy])
            lateral_low = min(
                lane_seg.direction * (lane_seg.radius - r_high),
    ...
    def evaluate_guard_disc(
        self, agent, discrete_variable_dict, continuous_variable_dict, track_map
    ):
        """
    ...
    def _evaluate_guard_disc(self, root, agent: BaseAgent, disc_var_dict, cont_var_dict, track_map):
        ...
        if isinstance(root, ast.Compare):
            ...
            for arg in root.args:
                res, new_arg = self._evaluate_guard_disc(
                    arg, agent, disc_var_dict, cont_var_dict, track_map
                )
    ...
    def evaluate_guard(self, agent, continuous_variable_dict, discrete_variable_dict, track_map):
        ...
        for i, node in enumerate(self.ast_list):
            tmp = self._evaluate_guard(
                node, agent, continuous_variable_dict, discrete_variable_dict, track_map
            )
            res = tmp and res
            if not res:
                break
    ...
    def _evaluate_guard(self, root, agent, cnts_var_dict, disc_var_dict, track_map):
        if isinstance(root, ast.Compare):
            ...
            right = self._evaluate_guard(
                root.comparators[0], agent, cnts_var_dict, disc_var_dict, track_map
            )
            if isinstance(root.ops[0], ast.GtE):
                return left >= right
            elif isinstance(root.ops[0], ast.Gt):
            ...
            if variable in cnts_var_dict:
                ...
                raise ValueError(
                    f"{variable} doesn't exist in either continuous varibales or discrete variables"
                )
        else:
            print("agent", agent)
            raise ValueError(f"Node type {root} from {unparse(root)} is not supported")
        ...
        for i in range(len(self.ast_list)):
            ...
            while j < sum(1 for _ in ast.walk(root)):
                ...
                if (
                    ...
                    and (node.func.id == "any" or node.func.id == "all")
                ):
    ...
    def _parse_elt(
        self, root, cont_var_dict, disc_var_dict, iter_name_list, targ_name_list, iter_pos_list
    ) -> Any:
        ...
        for i in range(len(self.ast_list)):
            ...
            for node in nodes:
                ...
                if isinstance(node, Reduction):
                    new_node = self.unroll_any_all_new(
                        node, cont_var_dict, disc_var_dict, len_dict, cont_var_updater
                    )
                    root = NodeSubstituter(node, new_node).visit(root)
            self.ast_list[i] = root
    ...
    def unroll_any_all_new(
        ...
        for i in range(len(iter_len_list)):
            ...
            iter_pos_list = iter_len_list[i]
            # substitute temporary variable in each of the elt and add corresponding variables in the variable dicts
            parsed_elt = self._parse_elt_new(
                changed_elt,
                cont_var_dict,
                disc_var_dict,
                cont_var_updater,
                    ...
                    if variable_name in cont_var_dict:
                        ...
                        if variable_name not in cont_var_updater:
                            ...
                            if (tmp_variable_name, iter_pos) not in cont_var_updater[variable_name]:
                                cont_var_updater[variable_name].append(
                                    (tmp_variable_name, iter_pos)
                                )
                    elif variable_name in disc_var_dict:
                        variable_val = disc_var_dict[variable_name][iter_pos]
                        disc_var_dict[tmp_variable_name] = variable_val
